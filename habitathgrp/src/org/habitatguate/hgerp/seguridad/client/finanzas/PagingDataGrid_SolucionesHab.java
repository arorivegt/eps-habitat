 ...
 * 
 */
public abstract class PagingDataGrid_SolucionesHab<T> extends Composite {
    ...
    public PagingDataGrid_SolucionesHab(List<T> dataList) {
    	this.dataList = dataList;
    	MyPaginationDataGrid_SolucionesHab.actual = 0;
    	if (!dataList.isEmpty()){
    	MyPaginationDataGrid_SolucionesHab.cantProduct = ((AuxSolucion)dataList.get(0)).getCostoProducto().size();
    	MyPaginationDataGrid_SolucionesHab.namesColumns = ((AuxSolucion)dataList.get(0)).getNombreProducto();
    	int plani = ((AuxSolucion)dataList.get(0)).getCostoProductoPlani().size();
    	if (plani > 0){
    		MyPaginationDataGrid_SolucionesHab.showplani = true;
    	}else{
    		MyPaginationDataGrid_SolucionesHab.showplani = false;
    	}
    	}
    	
        initWidget(dock);
        dataGrid = new DataGrid<T>(dataList.size(),
