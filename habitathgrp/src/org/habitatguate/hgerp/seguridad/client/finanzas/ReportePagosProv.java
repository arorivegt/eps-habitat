...
public class ReportePagosProv extends Composite   {
	...
	TablaGWT_ReportePagosProv e = null;
	...
	public ReportePagosProv() {
		...
		absolutePanel.add(listBox, 10, 43);
		...
		listEstado.addItem("[Seleccione Estado]","-1");
		...
		Label lblBusquedaPor = new Label("REPORTE DE PAGOS A PROVEEDORES");
		lblBusquedaPor.setStyleName("label");
		lblBusquedaPor.setSize("358px", "27px");
		...
		button_1.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				String estado = listEstado.getValue(listEstado.getSelectedIndex());
				...
				System.out.println("fecha ini"+fechaInicio+" fin: "+fechaFIn+" check"+checkRange+" estado"+estado+ " los valores de la siguiente: "+ idAfiliado + " "+ trimestre + " "+anio + " "+idProveedor);

				
				formPanel.setAction("/ExportReportePagosProv?idAfiliado="+idAfiliado+"&estado="+estado+"&trimestre="+trimestre+"&fechaInicio="
	...
	protected void BuscarPorFiltros() {
		...
		loginService.Consulta_PagosProvGenerica(idAfiliado, filter, idProveedor, anio, trimestre,fechaInicio,fechaFIn,estado,checkRange, new AsyncCallback<List<AuxHistorialPagoProv> >() {
			
			@Override
			public void onSuccess(List<AuxHistorialPagoProv>  result) {
				// TODO Auto-generated method stub
				System.out.println(result.size());
				
				if(!result.isEmpty()){
					e = new TablaGWT_ReportePagosProv(result);
					grid.setWidget(1, 0,e);
					e.setSize("1700px", "300px");
					}else{
						e = new TablaGWT_ReportePagosProv(new ArrayList<AuxHistorialPagoProv>());
						grid.setWidget(1, 0,e);
						e.setSize("1700px", "300px");
					}
			}
