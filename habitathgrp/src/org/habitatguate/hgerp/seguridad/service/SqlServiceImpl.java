...
public Long Insertar_MaterialCostruccionAfiliadoProveedor(Long idProveedor,String nomMaterialCostruccion,String unidadMetrica, Double precioUnitario, String idProducto,Long idAfiliado,String idCatalogoMaterial) throws IllegalArgumentException{
	 Long valor = 0L;
	final PersistenceManager gestorPersistencia = PMF.get().getPersistenceManager();
	try{
		Key k = new KeyFactory
	...
	nuevo.setProveedor(prov);
	nuevo.setIdCatalogoMaterial(idCatalogoMaterial);
...
//------------------------------------------------------CONSULTAS--------------------------------------------------	
	...
	public List<AuxProveedor> ConsultaDatosProveedor_Generico(String afiliado, String estado, String tipo){
		...
		if (!execute.isEmpty()){
			for (SegProveedor p : execute){
				...
				AuxProveedor aux = new AuxProveedor();
				aux.setAceptaCredito(p.getAceptaCredito());
				aux.setAceptaDonacion(p.getAceptaDonacion());
				aux.setAceptaExencion(p.getAceptaExencion());
				aux.setActividadEcono(p.getActividadEcono());
				aux.setAprobadoComision(p.getAprobadoComision());
				aux.setContribuyeEventos(p.getContribuyeEventos());
				aux.setCualesyComoEventos(p.getCualesyComoEventos());
				aux.setDepartamentos(p.getDepartamentos());
				aux.setDirProveedor(p.getDirProveedor());
				aux.setDisponibilidadProd(p.getDisponibilidadProd());
				aux.setFechaIngreso(ConvertDate.g(p.getFechaIngreso()));
				aux.setFormaDonacion(p.getFormaDonacion());
				aux.setFrecuenciaDonacion(p.getFrecuenciaDonacion());
				aux.setIdProveedor(p.getIdProveedor().getId());
				aux.setMontoMaximo(p.getMontoMaximo());
				aux.setMotivoInactivo(p.getMotivoInactivo());
				aux.setMunicipios(p.getMunicipios());
				aux.setNomProveedor(p.getNomProveedor());
				aux.setNumeroNit(p.getNumeroNit());
				aux.setObservacionDistribucion(p.getObservacionDistribucion());
				aux.setObservaciones(p.getObservaciones());
				aux.setPaginaWeb(p.getPaginaWeb());
				aux.setPersonaJuridica(p.getPersonaJuridica());
				aux.setPorcentDonacion(p.getPorcentDonacion());
				aux.setProductosfrece(p.getProductosfrece());
				aux.setRazonSocial(p.getRazonSocial());
				aux.setRegimenTributario(p.getRegimenTributario());
				aux.setRelacionConProv(p.getRelacionConProv());
				aux.setServicioEntrega(p.getServicioEntrega());
				aux.setTelProveedor(p.getTelProveedor());
				aux.setTiempoEntrega(p.getTiempoEntrega());
				aux.setTipoProveedor(p.getTipoProveedor());
				aux.setTipoProveedorGeneral(p.getTipoProveedorGeneral());
				aux.setUbicacionSucursales(p.getUbicacionSucursales());
				aux.setURLFileConvenio(p.getURLFileConvenio());
				aux.setKeyFileConvenio(p.getKeyFileConvenio());
				aux.setURLFileRTU(p.getURLFileRTU());
				aux.setKeyFileRTU(p.getKeyFileRTU());
	...
	public List<AuxValeBeneficiario> Consulta_MaterialCostruccionGenerica(String idAfiliado, String filter,String idProveedor, String anio, String trimestre, String fechaInicio, String fechaFIn, String idCatalogoMaterial,String idCatalogoMaterial2, boolean checkRange){
		...
		if (checkRange){
			query.setFilter("aprobado == 1 && fechaVale >= :dateTime  && fechaVale <= :dateTime2");
			execute = (List<SegVale>)query.execute(cal.getTime(),cal2.getTime());
		}else {
			query.setFilter("aprobado == 1");
			execute = (List<SegVale>)query.execute();
		}
		} catch (ParseException e) {
			// TODO Auto-generated catch block
						...
						if(detalleCompra.getMaterialCostruccion().getProveedor().getIdProveedor().getId() == Long.valueOf(idProveedor)|| idProveedor.equals("0"))  {
							if (detalleCompra.getSolucion().getAnio() == Integer.valueOf(anio) || anio.equals("0")){
								if (detalleCompra.getSolucion().getTrimestre() == Integer.valueOf(trimestre) || trimestre.equals("0")){
									...
									nuevo.setCantidadMat(detalleCompra.getCantidad());
									nuevo.setTotalPagar(detalleCompra.getSubTotal());
